...
<<<<<<< HEAD
        ...
        # Vector operations initialization
=======
...
app = FastAPI(
    ...
    description="RAG-powered PostgreSQL API with OpenAI & Claude",
    version="4.1.0",
=======
...
class ChatRequest(BaseModel):
    question: str = Field(..., description="User question", min_length=3, max_length=500)
    max_results: int = Field(default=3, description="Max retrieved documents", le=5, ge=1)
    similarity_threshold: float = Field(default=0.5, description="Similarity threshold", ge=0.3, le=0.9)
...
async def health_check():
    """API ve veritabanı health check - Enhanced with AI services status"""
=======
...
<<<<<<< HEAD
        ...
        try:
            ...
            if connection_test["success"]:
                ...
                claude_model = claude_service.default_model
            else:
                claude_status = f"error: {connection_test.get('message', 'unknown')}"
        except Exception as e:
...
<<<<<<< HEAD
# ===============================================
# AI CHAT ENDPOINTS (SEPARATED)
# ===============================================

@app.post("/chat/openai")
async def chat_with_openai(chat_request: ChatRequest):
    """RAG Chat with OpenAI"""
    
    try:
        logger.info(f"🔍 OpenAI RAG Query: '{chat_request.question}' with model: {chat_request.model}")
...
<<<<<<< HEAD
        logger.info(f"✅ OpenAI RAG response generated. Cost: ${openai_response.get('usage', {}).get('estimated_cost', 0):.4f}")
...
async def chat_with_claude(chat_request: ChatRequest):
    """RAG Chat with Claude"""
    
    try:
        logger.info(f"🔍 Claude RAG Query: '{chat_request.question}' with model: {chat_request.model}")
        ...
        logger.info(f"✅ Claude RAG response generated. Cost: ${claude_response.get('usage', {}).get('estimated_cost', 0):.6f}")
